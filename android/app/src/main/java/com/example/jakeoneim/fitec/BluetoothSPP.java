...
import static com.example.jakeoneim.fitec.BluetoothState.STATE_CONNECTED;
...
public class BluetoothSPP {
    ...
    public void connect(String address) {
        BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(address);
        mChatService.connect(device);
        Log.d("connect??????address",device.getAddress());
    }
    ...
    public void autoConnect(String keywordName) {
        if(!isAutoConnectionEnabled) {
            ...
            for(int i = 0 ; i < arr_name.length ; i++) {
                Log.d("pairedDivice",arr_address[i]);
            ...
            Log.d("ARR_FILTER_ADDRESS",arr_filter_address.get(0));
